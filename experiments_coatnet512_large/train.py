#!/usr/bin/env python3
"""Train the 4.25M CoAtNet at 512px with safe augmentation and multi-crop."""
...
from .augmentation import SafeGeoAugment512
from .model import CoAtNetLarge4M
from .multicrop import evaluate_multicrop


ROOT = Path(__file__).resolve().parent
MIN_PARAMETERS = 4_000_000
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--multicrop-size", type=int, default=448)
    parser.add_argument(
        "--allow-cpu",
        action="store_true",
        help="Allow CPU/MPS only for smoke testing",
    )
    return parser.parse_args()


def serializable_metrics(metrics) -> dict[str, object]:
    return metrics.serializable()
...
def write_markdown(report: dict[str, object], path: Path) -> None:
    ...
    valid_country = valid_multi["per_country_accuracy"]
...
def main() -> None:
    ...
    if not 256 <= args.multicrop_size < config.image_size:
        raise ValueError("multicrop size must be between 256 and 511")
    ...
    print("=" * 76)
    print("CoAtNet-Large-4M 512x512 — safe augmentation + 6-crop inference")
    print("=" * 76)
    ...
    print("No flips, rotations, coordinates, pretrained weights, or external data.")
    ...
    data.train.dataset.augment = SafeGeoAugment512(config.image_size)
    ...
    if not MIN_PARAMETERS <= parameters <= MAX_PARAMETERS:
        raise ValueError(
            f"Model has {parameters:,} parameters; expected 4M to {MAX_PARAMETERS:,}"
        )
    model.to(device)
    ...
    print("\nBest checkpoint loaded. Running deterministic final evaluation...")
    ...
    valid_single = evaluate(model, data.valid, criterion, device, data.countries)
    ...
    checkpoint_path = checkpoint_dir / "coatnet_large4m_512_best.pt"
    torch.save(
        {
            "model_name": "CoAtNet-Large-4M-512",
    ...
    report: dict[str, object] = {
        "experiment": "CoAtNet-Large-4M 512x512",
        "parameters": parameters,
        "best_epoch": best_epoch,
        "best_valid_accuracy": best_accuracy,
    ...
    print("\n" + "=" * 76)
    print("FINAL RESULTS — CoAtNet-Large-4M 512x512")
    print("=" * 76)
